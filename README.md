combat training custom difficulty and campaign to

EMZ/EMP zombies Nerf

all weapons upgrade similar to the Cell 3 Cauterizer

inside s1 make a scripts folder, then inside s1/scripts create 2 folders named mp and zm

for autobots.gsc put in the mp folder

for auto_bots_bo2_zombies.gsc and zm_emz.gsc and
zm_easy_round_scaling.gsc and zm_all_weapon_damage.gsc put them in zm folder

for custom names put Bots.txt next to where you made the scripts folder
