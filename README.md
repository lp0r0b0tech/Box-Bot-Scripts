This repository contains custom Box Bot and Exo Zombies script tweaks, and the install paths below apply only to the listed files.

EMZ/EMP Zombies Nerf

Mk1 weapons keep stock damage, and the explicit CB Servers S1x v0.0.4 Zombies internal weapon-name and alias list uses a Cell 3 Cauterizer-style damage curve from Mk2 through Mk25.

Inside `s1`, create a `scripts` folder. For the script files listed below, create `mp` and `zm` inside `s1/scripts`; no `sv` folder is needed for those script files.

Place `autobots_combat_training.gsc` at `s1/scripts/mp/autobots_combat_training.gsc`.

Place `zm_emz.gsc` at `s1/scripts/zm/zm_emz.gsc`.
Place `zm_all_weapon_damage.gsc` at `s1/scripts/zm/zm_all_weapon_damage.gsc`.

Place `Bots.txt` (the custom bot names file) directly under `s1` at `s1/Bots.txt`.
