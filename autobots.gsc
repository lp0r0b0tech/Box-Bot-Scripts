...
// - Maps custom ULTRA profile to native VETERAN bot skill
...
// Custom profile name.
// The engine-native skill used for "ultra" is "veteran".
...
// RAGE MODE profile.
// These are supplemental modifiers; native bot skill is controlled
// through botsetdifficulty().
rageBotAccuracy = 9.99;
rageBotReactionTime = 0.002;
...
init()
{
    ...
    // "ultra" is the custom script profile.
    // Native bot skill mapping is handled by resolveBotSkillDifficulty().
    lockedBotDifficulty = "ultra";
    defaultBotDifficulty = "ultra";
...
shouldRunAutobotsHere()
{
    if (isDefined(level.mapname))
    {
        ...
        if (isSubStr(mn, "exo survival") || isSubStr(mn, "exo zombies"))
            return false;

        if (isSubStr(mn, "cp_") || isSubStr(mn, "survival"))
            return false;

        if (isSubStr(mn, "zm_") || isSubStr(mn, "zombies"))
            return false;
    }
...
isBotEntity()
{
    ...
    warnOnce(
        ...
        "pers[\"isBot\"] missing for guid=" + guidFallback + "; using fallback checks"
    );
    ...
    if (!strictBotIdentityMode && isDefined(self.name))
    {
        ...
        if (isSubStr(nl, "bot ") || isSubStr(nl, "[bot]"))
            return true;
    }
...
// --------------------------
//
// The script uses "ultra" as its locked profile name.
// The native bot system uses the supported preset "veteran"
// for that profile.
//
// Native mapping:
//   recruit -> recruit
...
resolveBotSkillDifficulty(difficulty)
{
    ...
    warnOnce(
        "invalid_bot_skill",
        "Unknown difficulty '" + difficulty + "'; falling back to veteran"
    );
...
setBotDifficulty(difficulty)
{
    ...
    // Read the actual engine state before writing it.
    ...
    // Supplemental custom profile modifiers.
    // The actual native aiming/reaction skill is controlled above.
...
applyOpLoadout(ent)
{
    ...
    if (isDefined(opLethal) && opLethal != "")
        ent giveweapon(opLethal);

    if (isDefined(opTactical) && opTactical != "")
...
applyAutobotDifficulty(diff)
{
    ...
    // This stores the requested script profile.
    // The actual native state is checked with botgetdifficulty().
...
applyDifficultyToAllBots(forceWritePers)
{
    ...
    foreach (p in level.players)
    {
        ...
        {
            // This checks and repairs the actual native bot skill.
    ...
    for (;;)
    {
        if (debugAutobots && debugVerbose)
        {
            ...
            dbg(
                ...
                countTotalPlayersForCap() +
                " totalRaw=" +
                (isDefined(level.players) ? level.players.size : 0) +
                " humans=" +
                countHumans() +
                " bots=" +
                countBots() +
                ...
                lockedBotDifficulty +
                " nativeSkill=" +
                resolveBotSkillDifficulty(lockedBotDifficulty) +
                " dvar(bot_difficulty)=" +
...
run60SecondSanityTest()
{
    ...
    for (;;)
    {
        ...
        if (isDefined(level.time))
            elapsed = (level.time - startTime) / 1000.0;
        ...
        if (!level.combatTraining && total > target)
            overTargetFailures++;
        ...
        if (isDefined(level.players))
        {
            foreach (p in level.players)
            {
                ...
                // Verify the script profile marker.
                if (!isDefined(p.pers) ||
                    !isDefined(p.pers["autobot_diff_applied"]) ||
                    p.pers["autobot_diff_applied"] != expectedProfile)
                {
                    badBotDiffSeen++;
                    continue;
                }

                // Verify the actual native engine skill.
                actualDifficulty = p botgetdifficulty();
