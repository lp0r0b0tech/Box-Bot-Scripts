...
- `scr_es_autobots_reaction_time` (0.01-0.25, default 0.05) - bot think/reaction interval; lower is faster but may cost server performance if set too low
...
Purchases are conservative: bots require affordability and attempt interaction. For positive-cost purchases, success is confirmed only when score changes or weapon/ammo state changes after interaction; zero-cost interactions are treated as successful on interaction. The script never manually deducts score.
